...
@prefect.task()
def get_or_create_typesense_collection(
    collection_name, typesense_con, typesense_definition
):
...
class Params(BaseModel):
    ...
    typesense_host: str = Field(
        "typesense.acdh-dev.oeaw.ac.at", description="Host to use for typesense."
    )
    typesense_definition: dict | None = Field(
        None,
...
def push_data_to_typesense_flow(params: Params):
    ...
    collection_name = get_or_create_typesense_collection(
        params.typesense_collection_name, typesense_con, params.typesense_definition
    )
