...
from wikidata_index import (
    create_typesense_index_from_sparql_query,
...
class Params(BaseModel):
    path_sparql_query: str = Field(
        default="prosnet-prefect-pipelines/sparql/wikidata-city.sparql",
    ...
    incremental_date: int | None = Field(
        default=2, description="Number of days to retrieve update for (today - days)."
    )
    typesense_collection_name: str = Field(
        default="prosnet-wikidata-place-index",
    ...
    label_creator_function: str = Field(
        default="label_creator_place", description="Function to create the label field."
    )
...
if __name__ == "__main__":
    create_typesense_place_index_from_wikidata(
        Params(incremental_update=False, incremental_date=None)
    )
